...
public class FireBrandController : MonoBehaviour {
	...
	void Update () {
        ...
        else if (moveLag <= 0)
        {
            ...
            else if (!shielding)
            {
                ...
                {
                    if (xAxis < 0.3 && xAxis > -0.3) { xAxis = 0; }
                    if (yAxis < 0.3 && yAxis > -0.3) { yAxis = 0; }
                    ...
                    if (xAxis != 0 && yAxis != 0)
                    {
                        ...
                        //body.AddForce(new Vector2(xAxis, yAxis) * stats.airdodgeSpeed);
                        airdodgeTimer += stats.airDodgeInv;
                        ivTimer += stats.airDodgeLag;
                    }
                    else
                    {
                        //body.gravityScale = 0;
                        body.velocity = new Vector2(xAxis, yAxis * -1) * stats.airdodgeSpeed;
                        //body.AddForce(new Vector2(xAxis, yAxis * -1) * stats.airdodgeSpeed);
                        airdodgeTimer += stats.airDodgeInv;
                        ivTimer += stats.airDodgeLag;
                    }
                ...
                else if (xAxis > 0)
                {
                    ...
                    body.AddForce(new Vector2(xAxis, 0) * stats.maxAirSpeed * 4);
            ...
            if (hInput.GetButton("P1_Jump") && grounded)
            {
            ...
            if (hInput.GetButtonUp("P1_Jump") && grounded)
            {
            ...
            else if (hInput.GetButtonDown("P1_Jump") && jumpsLeft > 0 && !grounded)
            {
                myAnimator.SetTrigger("jump");
                myAnimator.SetBool("grounded", false);
            ...
            if (hInput.GetButton("P1_Shield") && grounded)
            {
                Shield();
                myAnimator.SetBool("grounded", true);
            ...
            if (hInput.GetButtonUp("P1_Shield") && grounded)
            {
                myAnimator.SetBool("grounded", true);
                rolled = false;
                ShieldOff();
    ...
    // TODO: implement invincibility check to the attacks, and turn off freefall
