...
public class samus_P2_CharController : MonoBehaviour
{
    ...
    void Start()
    {
        //sw = new StreamWriter("stats2.txt", true);
        ...
        timeInAttack = 0;
        playerAudio = gameObject.GetComponent<AudioSource>(); // players audio source 
        ...
        int ch = (MatchPref.player1 == 0) ? testChar : MatchPref.player2;
        switch (ch)
        {
            case 1:
                attacks = gameObject.GetComponent<P2_FB_Attacks>();
                break;
            case 2:
                attacks = gameObject.GetComponent<P2_Link_Attacks>();
                break;
            case 3:
                attacks = gameObject.GetComponent<P2_Mario_Attacks>();
                break;
            case 4:
                //Debug.Log("loading samus attacks");
                attacks = gameObject.GetComponent<P2_Samus_Attacks>();
                break;
        }
    ...
    void GroundedUpdate()
    {
        ...
        if (hInput.GetButtonUp("P2_Shield"))
        {
            myAnimator.SetBool("grounded", true);
            rolled = false;
            ShieldOff();
            ...
            else
            {
                ...
                else if (yAxis > 0)
                {
                    ...
                    else if (hInput.GetButtonDown("P2_Special"))
                    {
                        ...
                        {
                            stats.charState = CharacterStats.state.charging;
                            myAnimator.SetTrigger("nSpecial");
                            playerAudio.clip = audio_charging;
                            playerAudio.Play();
                        }
                        return;
                    }
    ...
    void AirborneUpdate()
    {
        ...
        // Manage Airdodges 
        if (hInput.GetButtonDown("P2_Shield"))
        {
        ...
        // Manage extra jumps
        if (hInput.GetButtonDown("P2_Jump") && jumpsLeft > 0)
        {
            myAnimator.SetTrigger("jump");
            numJumps++;
            playerAudio.clip = audio_jump;
    ...
    {
        charging = true; //new
        chargePercent += Time.deltaTime; //new
        ...
        // new
        if (Mathf.Abs(hInput.GetAxis("P2_Horizontal")) > deadZone && grounded)
        {
            myAnimator.SetTrigger("finishedCharging");
            myAnimator.SetTrigger("dodge");
            rolled = true;
        ...
        if (hInput.GetButtonDown("P2_Jump"))
        {
            myAnimator.SetTrigger("finishedCharging");
            myAnimator.SetTrigger("jump");
            numJumps++;
        ...
        if (hInput.GetButtonDown("P2_Special"))
        {
            stats.charState = CharacterStats.state.attacking;
            preAttackTimer = stats.nSpecialStartup;
            activeAttackTimer = stats.nSpecialActive;
    ...
    private void OnCollisionStay2D(Collision2D c)
    {
        ...
        {
            if (hInput.GetAxis("P2_Vertical") > dropThresh)
            {
                foreach (Transform t in gameObject.GetComponentsInChildren<Transform>(true))
                {
