...
using System;
using System.IO;
public class samus_P1_CharController : MonoBehaviour
{
    ...
    void Start()
    {
        //sw = new StreamWriter("stats1.txt", true);
    ...
    void ChargeUpdate()
    {
        ...
        // new
        if (Mathf.Abs(hInput.GetAxis("P1_Horizontal")) > deadZone && grounded)
        {
            myAnimator.SetTrigger("finishedCharging");
            myAnimator.SetTrigger("dodge");
            rolled = true;
        ...
        if (hInput.GetButtonDown("P1_Jump"))
        {
            myAnimator.SetTrigger("finishedCharging");
            myAnimator.SetTrigger("jump");
            numJumps++;
        ...
        if (hInput.GetButtonDown("P1_Special"))
        {
            stats.charState = CharacterStats.state.attacking;
            preAttackTimer = stats.nSpecialStartup;
            activeAttackTimer = stats.nSpecialActive;
