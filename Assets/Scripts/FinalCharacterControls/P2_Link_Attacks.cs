...
public class P2_Link_Attacks : CharAttacks
{
    ...
    public override void ForwardAir()
    {
        ...
        {
            cols = Physics2D.OverlapBoxAll(stats.hitBoxes[1].bounds.center, stats.hitBoxes[1].bounds.extents, 0.0f, LayerMask.GetMask("Player1"));
        }
        else if (controller.timeInAttack < fAttackTimes[2])
        {
            cols = Physics2D.OverlapBoxAll(stats.hitBoxes[2].bounds.center, stats.hitBoxes[2].bounds.extents, 0.0f, LayerMask.GetMask("Player1"));
        }
        else
        {
    ...
    public override void SideSpecial()
    {
        ...
        GameObject[] bombs = GameObject.FindGameObjectsWithTag("bomb2");
        ...
        bomb.GetComponent<Bomb>().enemyTag = "Player1";
    ...
    public override void Special()
    {
        ...
        arrow.GetComponent<Arrow>().enemyTag = "Player1";
    ...
    public override void UpSpecial()
    {
        ...
        Collider2D[] cols = Physics2D.OverlapBoxAll(stats.hitBoxes[13].bounds.center, stats.hitBoxes[13].bounds.extents, 0.0f, LayerMask.GetMask("Player1"));
        foreach (Collider2D c in cols)
        {
            if (c.gameObject.tag.Equals("Player1"))
            {
